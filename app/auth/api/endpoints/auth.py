...
def register_user(user: User_Register, session=Depends(get_session)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Erreur lors de l'enregistrement: {str(e)}")
...
def login(user: User_login, session=Depends(get_session)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Échec de la génération du token")
